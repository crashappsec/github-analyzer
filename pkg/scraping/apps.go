...
func (c *Client) AppRestrictionsEnabled(org string) (bool, error) {
	doc, err := c.get(
		"/organizations/%s/settings/oauth_application_policy",
		org,
	)
	if err != nil {
		return false, err
...
func (c *Client) ListOAuthApps(org string) ([]OAuthApp, error) {
	...
	var apps []OAuthApp
	doc.Find(".oauth-application-allowlist ul > li").
		Each(func(i int, s *goquery.Selection) {
			var app OAuthApp
			app.Name = s.Find(".request-info strong").First().Text()
			app.Description = s.Find(".request-info .application-description").
				Text()
